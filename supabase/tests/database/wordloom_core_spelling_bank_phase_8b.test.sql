...
create temporary table phase_8b_expected_targets (
  focus_grapheme text primary key,
  expected_primary_word_count integer not null
) on commit drop;

insert into phase_8b_expected_targets (focus_grapheme, expected_primary_word_count)
values
  ('ear', 22),
  ('oy', 28),
  ('oi', 30),
...
create temporary table phase_8b_words on commit drop as
select *
...
create temporary table phase_8b_primary_coverage on commit drop as
select
  word_targets.focus_grapheme,
  count(distinct words.id)::integer as word_count
from phase_8b_words as words
...
select is((select count(*)::integer from phase_8b_words), 250, 'Phase 8B has exactly 250 active core words');

select is(
  (
    select count(*)::integer
    from phase_8b_expected_targets as expected
    inner join phase_8b_primary_coverage as actual
      on actual.focus_grapheme = expected.focus_grapheme
     and actual.word_count = expected.expected_primary_word_count
  ),
  9,
  'Phase 8B per-target primary counts match expected coverage'
);
...
select is(
  (
    ...
    inner join phase_8b_expected_targets as expected
      on expected.focus_grapheme = targets.focus_grapheme
    where targets.is_active is true
  ),
  9,
  'Phase 8B targets exist and are active'
);

select ok(
  not exists (
    select 1
    from phase_8b_words as phase_words
  ...
  'Phase 8B rows do not duplicate earlier production words'
);
...
select ok(
  ...
  'every Phase 8B word has sentence and meaning'
);
...
select ok(
  ...
  'every Phase 8B word is approved, suitable, active, and Wordloom-owned'
);
...
select ok(
  ...
  'Phase 8B words do not contain placeholder context'
);
...
select ok(
  ...
  'Phase 8B words do not contain explicit spelling-hint context'
);
...
select ok(
  ...
  'Phase 8B words have non-empty grapheme and focus arrays'
);
...
select ok(
  ...
  'Phase 8B grapheme_segments reconstruct normalised_word'
);
...
select ok(
  ...
  'Phase 8B primary_focus_grapheme appears in focus_graphemes'
);
...
select ok(
  ...
  'Phase 8B primary_focus_grapheme appears in grapheme_segments'
);
...
select ok(
  ...
  'every Phase 8B word has exactly one primary target link'
);
...
select is(
  ...
  250,
  'Phase 8B has exactly 250 primary word-target links'
);
...
select ok(
  ...
  'Phase 8B target_role values are valid'
);
...
select ok(
  ...
  'Phase 8B links point to active matching focus targets'
);
...
select is(
  ...
  0,
  'active core words have no duplicate normalised words after Phase 8B'
);
...
select ok(
  not exists (
    ...
    inner join phase_8b_words as words
      on words.id = overrides.core_word_id
  ),
  'Phase 8B does not create school override rows for new core words'
);
...
select ok(
  ...
  'Phase 8B does not add rows to school spelling bank additions'
);
...
select is(
  ...
  1250,
  'Wordloom core production bank has exactly 1,250 active approved suitable words after Phase 8B'
);
