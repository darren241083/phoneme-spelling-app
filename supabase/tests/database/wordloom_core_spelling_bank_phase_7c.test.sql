...
insert into phase_7c_expected_targets (focus_grapheme, expected_primary_word_count)
values
  ('aw', 8),
  ('ure', 8),
  ('ay', 4),
...
create temporary table phase_7c_words on commit drop as
select *
...
create temporary table phase_7c_primary_coverage on commit drop as
select
  word_targets.focus_grapheme,
  count(distinct words.id)::integer as word_count
from phase_7c_words as words
...
select is(
  (select count(*)::integer from phase_7c_words),
  40,
  'Phase 7C has exactly 40 active core words'
);

select is(
  (
    select count(*)::integer
    from phase_7c_expected_targets as expected
    inner join phase_7c_primary_coverage as actual
      on actual.focus_grapheme = expected.focus_grapheme
     and actual.word_count = expected.expected_primary_word_count
  ),
  8,
  'Phase 7C per-target primary counts match expected coverage'
);
...
select is(
  (
    ...
    where focus_grapheme in ('aw', 'ure')
      and is_active is true
  ),
  2,
  'Phase 7C launch targets aw and ure exist and are active'
);
...
select is(
  (
    select count(*)::integer
    from public.wordloom_core_focus_targets
    where focus_grapheme in ('ay', 'ea', 'ew', 'tch', 'air', 'au')
      and is_active is true
  ),
  6,
  'Phase 7C top-up targets remain active'
);

select ok(
  not exists (
    select 1
    from phase_7c_words as phase_words
  ...
  'Phase 7C rows do not duplicate proof words'
);
...
select ok(
  ...
  'Phase 7C rows do not duplicate Phase 7B words'
);
...
select ok(
  ...
  'every Phase 7C word has sentence and meaning'
);
...
select ok(
  ...
  'every Phase 7C word is approved, suitable, active, and Wordloom-owned'
);
...
select ok(
  ...
  'Phase 7C words do not contain placeholder context'
);
...
select ok(
  ...
  'Phase 7C words do not contain explicit spelling-hint context'
);
...
select ok(
  ...
  'Phase 7C words have non-empty grapheme and focus arrays'
);
...
select ok(
  ...
  'Phase 7C grapheme_segments reconstruct normalised_word'
);
...
select ok(
  ...
  'Phase 7C primary_focus_grapheme appears in focus_graphemes'
);
...
select ok(
  ...
  'Phase 7C primary_focus_grapheme appears in grapheme_segments'
);
...
select ok(
  ...
  'every Phase 7C word has exactly one primary target link'
);
...
select is(
  ...
  40,
  'Phase 7C has exactly 40 primary word-target links'
);
...
select ok(
  ...
  'Phase 7C target_role values are valid'
);
...
select ok(
  ...
  'Phase 7C links point to active matching focus targets'
);
...
select is(
  ...
  0,
  'active core words have no duplicate normalised words after Phase 7C'
);
...
select ok(
  not exists (
    ...
    inner join phase_7c_words as words
      on words.id = overrides.core_word_id
  ),
  'Phase 7C does not create school override rows for new core words'
);
...
select ok(
  ...
  'Phase 7C does not add rows to school spelling bank additions'
);
