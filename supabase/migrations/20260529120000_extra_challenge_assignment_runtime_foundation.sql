...
set evidence_source = 'assigned_core'
where evidence_source is null
   or btrim(evidence_source) = '';

alter table public.assignments_v2
  alter column evidence_source set default 'assigned_core',
  alter column evidence_source set not null;

alter table public.assignments_v2
  drop constraint if exists assignments_v2_evidence_source_check;

alter table public.assignments_v2
  add constraint assignments_v2_evidence_source_check
  check (evidence_source in ('assigned_core', 'extra_challenge'));

create index if not exists idx_assignments_v2_school_evidence_created
  on public.assignments_v2 (school_id, evidence_source, created_at desc);
...
declare
  ...
  previous_check text := $previous$
      ...
      and (
$previous$;
  patched_check text := $patched$
      and lower(btrim(coalesce(assignment.automation_kind, ''))) = 'personalised'
      and lower(btrim(coalesce(assignment.automation_source, ''))) = 'manual_run_now'
      and coalesce(nullif(lower(btrim(coalesce(assignment.evidence_source, ''))), ''), 'assigned_core') = 'assigned_core'
      and (
$patched$;
begin
  select pg_get_functiondef('public.claim_waiting_personalised_generation_run_pupil(uuid)'::regprocedure)
    into function_sql;

  if position('assignment.evidence_source' in function_sql) > 0 then
    return;
  end if;

  if position(previous_check in function_sql) = 0 then
    raise exception 'Could not patch claim_waiting_personalised_generation_run_pupil active assignment guard for extra challenge foundation.';
  end if;

  function_sql := replace(function_sql, previous_check, patched_check);
