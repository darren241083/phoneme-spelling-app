...
begin
  ...
  select
    ...
    into test_school_id, test_status
  ...
  if tg_op = 'INSERT' or (tg_op = 'UPDATE' and new.test_id is distinct from old.test_id) then
    ...
    if coalesce(lower(btrim(new.automation_kind)), '') <> 'personalised' then
      select coalesce(
        bool_and(
