...
declare
  ...
  completed_baseline_assignment_id uuid := null;
...
begin
  ...
  with baseline_assignments as (
    select
      a.id,
      a.created_at,
      count(tw.id)::integer as baseline_word_count,
      bool_or(public.is_standard_baseline_choice(tw.choice, safe_required_key)) as matches_required_standard
    ...
    having count(tw.id) > 0
      and bool_and(
        public.is_standard_baseline_choice(tw.choice, safe_required_key)
        or public.is_standard_baseline_choice(tw.choice, 'core_v2')
        or public.is_standard_baseline_choice(tw.choice, 'core_v1')
        or lower(btrim(coalesce(tw.choice ->> 'source', ''))) in ('baseline_v1', 'baseline_v2')
        or lower(btrim(coalesce(tw.choice ->> 'baseline_v1', ''))) in ('true', '1', 'yes')
        or lower(btrim(coalesce(tw.choice ->> 'baseline_v2', ''))) in ('true', '1', 'yes')
      )
  ),
  completed_from_status as (
    select
      ba.id,
      ba.created_at,
      aps.completed_at as completed_at,
      ba.matches_required_standard
    from baseline_assignments as ba
    inner join public.assignment_pupil_statuses as aps
      on aps.assignment_id = ba.id
     and aps.pupil_id = requested_pupil_id
     and aps.completed_at is not null
  ),
  completed_from_attempts as (
    select
      ba.id,
      ba.created_at,
      max(at.created_at) as completed_at,
      ba.matches_required_standard
    from baseline_assignments as ba
    inner join public.test_words as tw
      on tw.test_id = (
    ...
    inner join public.attempts as at
      on at.assignment_id = ba.id
     and at.pupil_id = requested_pupil_id
     and at.test_word_id = tw.id
    group by
      ba.id,
      ba.created_at,
      ba.baseline_word_count,
      ba.matches_required_standard
    having count(distinct at.test_word_id) >= ba.baseline_word_count
  ),
  completed_baselines as (
    select * from completed_from_status
    union all
    select * from completed_from_attempts
  )
  select candidate.id
  into completed_baseline_assignment_id
  from completed_baselines as candidate
  order by
    case when candidate.matches_required_standard then 0 else 1 end,
    candidate.completed_at desc nulls last,
    candidate.created_at desc nulls last,
    candidate.id desc
  limit 1;

  if completed_baseline_assignment_id is not null then
    return jsonb_build_object(
      'status', 'ready',
      'waiting_reason', null,
      'assignment_id', null,
      'completed_assignment_id', completed_baseline_assignment_id,
      'required_standard_key', safe_required_key,
      'class_ids', to_jsonb(active_class_ids),
      'form_class_ids', to_jsonb(active_form_class_ids),
      'assignment', null
  ...
  order by
    case when candidate.completed_at is not null then 0 else 1 end,
    candidate.created_at desc nulls last,
    candidate.end_at asc nulls last,
    candidate.id desc
  limit 1;
