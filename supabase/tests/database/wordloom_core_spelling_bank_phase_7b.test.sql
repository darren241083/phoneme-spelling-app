...
create temporary table phase_7b_expected_targets (
  focus_grapheme text primary key,
  expected_primary_word_count integer not null
) on commit drop;

insert into phase_7b_expected_targets (focus_grapheme, expected_primary_word_count)
values
  ('ay', 8),
  ('ea', 8),
  ('ew', 8),
...
create temporary table phase_7b_words on commit drop as
select *
...
create temporary table phase_7b_primary_coverage on commit drop as
select
  word_targets.focus_grapheme,
  count(distinct words.id)::integer as word_count
from phase_7b_words as words
...
select is(
  (select count(*)::integer from phase_7b_words),
  40,
  'Phase 7B has exactly 40 active core words'
);

select is(
  (
    select count(*)::integer
    from phase_7b_expected_targets as expected
    inner join phase_7b_primary_coverage as actual
      on actual.focus_grapheme = expected.focus_grapheme
     and actual.word_count = expected.expected_primary_word_count
  ),
  6,
  'Phase 7B per-target primary counts match expected coverage'
);
...
select is(
  (
    ...
    where focus_grapheme in ('ay', 'ea', 'ew', 'tch')
      and is_active is true
  ),
  4,
  'Phase 7B launch targets ay, ea, ew, and tch exist and are active'
);
...
select is(
  (
    select count(*)::integer
    from public.wordloom_core_focus_targets
    where focus_grapheme in ('air', 'au')
      and is_active is true
  ),
  2,
  'Phase 7B top-up targets air and au remain active'
);

select ok(
  not exists (
    select 1
    from phase_7b_words as phase_words
  ...
  'Phase 7B top-up rows do not duplicate proof words'
);
...
select ok(
  ...
  'every Phase 7B word has sentence and meaning'
);
...
select ok(
  ...
  'every Phase 7B word is approved, suitable, active, and Wordloom-owned'
);
...
select ok(
  ...
  'Phase 7B words do not contain placeholder context'
);
...
select ok(
  ...
  'Phase 7B words have non-empty grapheme and focus arrays'
);
...
select ok(
  ...
  'Phase 7B grapheme_segments reconstruct normalised_word'
);
...
select ok(
  ...
  'Phase 7B primary_focus_grapheme appears in focus_graphemes'
);
...
select ok(
  ...
  'Phase 7B primary_focus_grapheme appears in grapheme_segments'
);
...
select ok(
  ...
  'every Phase 7B word has exactly one primary target link'
);
...
select is(
  ...
  40,
  'Phase 7B has exactly 40 primary word-target links'
);
...
select ok(
  ...
  'Phase 7B target_role values are valid'
);
...
select ok(
  ...
  'Phase 7B links point to active matching focus targets'
);
...
select is(
  ...
  0,
  'active core words have no duplicate normalised words after Phase 7B'
);
...
select ok(
  not exists (
    ...
    inner join phase_7b_words as words
      on words.id = overrides.core_word_id
  ),
  'Phase 7B does not create school override rows for new core words'
);
...
select ok(
  ...
  'Phase 7B does not add rows to school spelling bank additions'
);
